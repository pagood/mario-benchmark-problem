...
public class Enemy extends Sprite implements Cloneable
{
    ...
    public Enemy(LevelScene world, float x, float y, int dir, int type, boolean winged, int mapX, int mapY)
    {
        ...
        this.x = x;
        this.y = y;
        avoidCliffs = type == competition.cig.yuxiao.sprites.Enemy.ENEMY_RED_KOOPA;

        noFireballDeath = type == competition.cig.yuxiao.sprites.Enemy.ENEMY_SPIKY;
    ...
    public void collideCheck()
    {
        ...
        if (xMarioD > -width*2-4 && xMarioD < width*2+4)
        {
            ...
            {
                if (type != competition.cig.yuxiao.sprites.Enemy.ENEMY_SPIKY && world.mario.ya > 0 && yMarioD <= 0 && (!world.mario.onGround || !world.mario.wasOnGround))
                {
                    ...
                    else
                    {
                        ...
                        if (type == competition.cig.yuxiao.sprites.Enemy.ENEMY_RED_KOOPA)
                        {
                            spriteContext.addSprite(new Shell(world, x, y, 0));
                        }
                        else if (type == competition.cig.yuxiao.sprites.Enemy.ENEMY_GREEN_KOOPA)
                        {
